...
Требуется: Python 3.12+, Go 1.22+, Node 20+. Если чего-то нет, `setup.ps1`
подскажет команду установки.

```bash
git clone https://github.com/leonttev/match-predictor.git
cd match-predictor
powershell -ExecutionPolicy Bypass -File scripts\setup.ps1
powershell -ExecutionPolicy Bypass -File scripts\run.ps1
```

`setup.ps1` собирает Go-модуль, создаёт виртуальные окружения Python и
устанавливает зависимости; `run.ps1` поднимает все четыре компонента, каждый
в своём окне, и проверяет их доступность. Остановить всё: `scripts\stop.ps1`.

Используемые порты: 8090 — функциональный модуль, 8081 — модуль работы с БД,
8000 — бэкенд, 5173 — фронтенд. Если какой-то из них занят посторонней
программой, его можно заменить — остальные компоненты узнают об этом
автоматически:

```bash
powershell -ExecutionPolicy Bypass -File scripts\run.ps1 -BackendPort 8100
powershell -ExecutionPolicy Bypass -File scripts\stop.ps1 -BackendPort 8100
```

После запуска открыть `http://localhost:5173`, зарегистрироваться и нажать
«Обновить данные и пересчитать рейтинги» — база в репозиторий не входит, и на
новой машине она пустая. Загрузка занимает около 25 секунд и **требует
интернета**. Если на месте демонстрации интернета может не быть, загрузите
данные заранее либо скопируйте файл `db-service/data.db` с машины, где данные
уже загружены.

## Запуск вручную (по компонентам)
